...
public interface ApostaRepository extends JpaRepository<Aposta, Long> {
	 ...
	 long countByStatus(StatusAposta status);
	...
	List<Aposta> findByPartida(Partida partida);
	
	@Query("SELECT COUNT(a) FROM Aposta a WHERE a.status = :status")
	long contarApostasPorStatus(StatusAposta status);
